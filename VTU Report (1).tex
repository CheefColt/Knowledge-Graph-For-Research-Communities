...
We extend our sincere thanks to our Principal, textbf{Mr.Sanjay Jain}, for providing an environment that fosters innovation and learning. His unwavering support and encouragement have been instrumental in the successful completion of this project.
...
This work presents the development and implementation of a knowledge graph-based system for identifying and analyzing the relationships considering a dataset of scopus publications belonging to an organization. Further it generates a knowledge graph using the same dataset and addresses the challenge of understanding complex collaboration patterns and predicting future research partnerships by transforming bibliographic metadata into structured graph representations. Using a dataset of 1,691 research papers encompassing 3,057 authors and 665 publication venues, the system constructs a comprehensive collaboration network containing 5 different versions which include : Author, Paper, Journal Network (LIMIT 20) - shows the basic structure of how academic publishing works using three layers, Author, Paper, Journal Network (LIMIT 100) - is an expansion of the previous network offering a wider view of the academic world, keeping the same three main types of nodes but carrying more information, Community Detection (LIMIT 100) - a network of co-authorships that is modular, with clear separate groups of researchers who collaborate together, Community Detection (LIMIT 300) - shows a more complex structure where we can observe both tightly connected groups and loosely linked collaboration networks, with varying levels of connection between them, Paper and Publication Venue Network - reveals the publication landscape across multiple academic domains which act as key hubs that aggregate research from diverse sources. All of these in Neo4j graph database, where authors, papers, and venues are represented as interconnected nodes. The implementation employs the Louvain algorithm for community detection, successfully identifying 847 research communities with a modularity score of 0.73, indicating strong organizational structure within the network. Additionally, a hybrid similarity-based approach combining Cosine and Jaccard metrics enables link prediction, identifying potential future link formation between researchers who share similar publication patterns. The system demonstrates how graph database technology and network analysis algorithms can provide valuable insights into research community dynamics, facilitate link prediction discovery, and support strategic decision-making for researchers, academic institutions, and funding organizations seeking to understand and optimize academic collaboration networks. We can extend the same work by generating knowledge embeddings which can be further used for model training using machine learning. We can generate such knowledge graphs real time using scopus APIs and extend our work.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 2: Related Work} – Reviews key academic papers and existing technologies relevant to our domain.
    \item \textbf{Chapter 3: Proposed Model} – Presents the initial design strategy and planned components.
    \item \textbf{Chapter 4: Implementation} – Provides detailed implementation specifics including preprocessing, graph construction, and algorithm execution.
    \item \textbf{Chapter 5: Results} – Presents quantitative and qualitative findings from the analysis.
    \item \textbf{Chapter 6: Sample Cypher Queries} – A List of several cypher queries which demonstrate different information that can be extracted from the Neo4j database.
    \item \textbf{Chapter 7: Conclusion and Future Scope} – Summarizes outcomes, contributions and outlines potential enhancements and research directions.
\end{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.55\textwidth]{flowgraph.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.2\textwidth]{media/Picture1.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.3\textwidth]{media/Picture2.png}
