...
\usepackage{ulem} % Add this in the preamble
\usepackage[a4paper, top=3cm, bottom=2.5cm, left=2.5cm, right=2.5cm]{geometry}
\usepackage{everypage} % To ensure the border is applied to all pages
% \usepackage{hyperref} % For \href and hyperlinks
\usepackage{lipsum} % For dummy text (remove in actual document)
\usepackage{amsmath}
\usepackage{anyfontsize}
\usepackage{ragged2e} % To enable justification
\usepackage{url}
\usepackage{float} 


% Border for all pages - TEMPORARILY DISABLED TO AVOID COMPILE TIMEOUT
% Uncomment this block for final PDF with borders
% \AddEverypageHook{%
%     \begin{tikzpicture}[remember picture,overlay]
%         % Adjust the border to fit within the specified margins
%         \draw[thick] 
...
% Background setup for logo watermark - TEMPORARILY DISABLED TO AVOID COMPILE TIMEOUT
% Uncomment this block for final PDF with watermark
% \backgroundsetup{
%   scale=0.8,
%   angle=0,
%   opacity=0.1,
...
%         {\includegraphics[width=15cm]{cmrit_logo.png}};
%     \end{tikzpicture}
%   }
% }

\pagestyle{fancy} % Use fancy header/footer
\fancyhf{} % Clear header/footer
\renewcommand{\headrulewidth}{0pt} % No header line
\renewcommand{\footrulewidth}{0pt} % No footer line

% Justify content globally
\justifying
...
% \setlength{\baselineskip}{20pt} % Adjust the line spacing manually
% {\fontsize{20}{15}\selectfont \textbf{VISVESVARAYA TECHNOLOGICAL UNIVERSITY}}

% {\fontsize{15}{15}\selectfont \textbf{BELAGAVI-590018,KARNATAKA}}
...
% {\fontsize{14}{0}\selectfont \underline{\textbf{PROJECT REPORT ON}}} 

% \vspace{0.5cm}

% {\fontsize{18}{1}\selectfont \textbf{"Knowledge Graph Generation for Research Communities (Phase 1)"}}

% \vspace{0.5cm}

% {\fontsize{14}{0}\selectfont \textit{Submitted in fullfillment for the requirement of the degree of}} 

% \vspace{0.2cm}


% {\fontsize{16}{0}\selectfont \textbf{BACHELOR OF ENGINEERING \\ 
% \vspace{0.2cm}
% IN \\
% INFORMATION SCIENCE \& ENGINEERING}} \\

% {\fontsize{12}{0} {\underline{Submitted By}}}
...
% {\fontsize{12}{0} \textbf{Under the Guidance of}}
...
% \begin{tikzpicture}
% \node
%     {\includegraphics[width=5cm]{cmrit_logo.png}};
% \end{tikzpicture}

% {\fontsize{12}{0}Information Science \& Engineering}

% {\fontsize{12}{0}CMR Institute of Technology}

% {\fontsize{12}{0}Sri Nivasa Reddy Layout, AECS Layout, Marathahalli, Bengaluru, Karnataka 560037}

% {\fontsize{12}{0}2024 - 2025}
...
\begin{center}
{\fontsize{16}{0} \textbf{{ACKNOWLEDGEMENT}}}
...
So, it is with gratitude we acknowledge all those whose guidance and encouragement served as beacon of light and crowned our effort with success.

\vspace{0.5cm}


We would like to express our gratitued towards \textbf{Dr. Jagadishwari V}, Professor and HOD, Department of Information Science \& Engineering who provided guidance and gave valuable suggestions regarding the project.

\vspace{0.5cm}


We consider it a privilege and honor to express our sincere gratitude to our internal guide,\textbf{ Dr. Suruchi Sabherwal }, \textbf{Assistant Professor}, Department of Information Science \& Engineering, for her valuable guidance throughout the tenure of this project work.
...
\noindent This report outlines the Phase 1 progress of a two-phase project aimed at developing a system to extract and visualize knowledge graphs from research papers. With the rapid growth of academic literature, it has become increasingly difficult for researchers to manually organize and interpret unstructured research content. This project addresses that challenge by exploring the use of AI-based techniques to automate the extraction of key entities and relationships from academic PDFs, ultimately converting them into structured, queryable knowledge graphs.
\\ \\
The focus of Phase 1 is to conduct an in-depth literature review, evaluate relevant tools and models, and establish a clear roadmap for implementation. Various approaches were studied, including PDF parsing libraries (like pdfminer.six), transformer-based LLMs (such as GPT, Gemini, and Hugging Face pipelines), and graph construction tools (like NetworkX and Neo4j). Experimental trials were conducted to test how well language models can extract structured metadata from research text, revealing both potential and limitations.
\\ \\
A proposed system pipeline has been designed based on the findings of this research phase. It involves parsing papers, extracting entities and relationships, and rendering them as nodes and edges in a dynamic knowledge graph. This graph would support natural language queries and enable researchers to interactively explore academic content across domains.
\\ \\
Phase 1 concludes with a clear direction for development in Phase 2, which will involve the actual implementation and testing of the system. By grounding the project in existing research and identifying viable technologies, this phase lays the foundation for building an intelligent tool that can assist with scientific knowledge discovery and organization.
...
Despite the availability of vast digital repositories of research papers, extracting meaningful, structured knowledge from them remains a significant challenge. Most academic articles are distributed in PDF format, which lacks semantic markup and is optimized for human reading rather than machine interpretation. As a result, information such as research objectives, methodologies, datasets, and conclusions is embedded in free-form text, making it difficult to retrieve, relate, or reason over automatically.
\\ \\
Current tools and frameworks either focus on surface-level metadata extraction (e.g., title, authors, and citations) or are highly domain-specific, limiting their generalizability. Additionally, they often require considerable manual setup, lack support for visual or semantic exploration, and do not integrate well with modern language models. There is no unified pipeline that takes an academic PDF as input and outputs a graph-based structure capturing the paper’s core concepts and relationships. Therefore, the core problem this project seeks to address is the lack of an accessible, intelligent system capable of transforming unstructured research papers into structured, queryable knowledge graphs. Phase 1 focuses on identifying the right models, techniques, and tools needed to solve this problem effectively.



\section{Scope of the Project}
This project is structured in two distinct phases across two academic semesters. Phase 1, which is the focus of this report, involves conducting a comprehensive literature review on existing methods and technologies used in knowledge graph construction, research document processing, and AI-based information extraction. It also includes analyzing available tools for PDF parsing, entity recognition, and graph visualization, as well as evaluating the capabilities of different language models such as OpenAI's GPT, Google Gemini, and Hugging Face pipelines. The outcome of this phase is a proposed design strategy and a clear direction for the implementation work that will be undertaken in Phase 2.
\\ 
...
This project addresses the following key research questions that guide the development and evaluation of our knowledge graph-based collaboration network:

\begin{enumerate}
    \item \textbf{How can bibliographic metadata be effectively transformed into a knowledge graph that captures collaboration patterns?} This involves determining the optimal graph schema, entity resolution strategies for author disambiguation, and relationship modeling approaches that balance expressiveness with computational tractability.
    
    \item \textbf{Which community detection algorithms best reveal meaningful research communities in academic collaboration networks?} We investigate whether modularity-based methods like Louvain accurately identify cohesive research groups and how community structure relates to disciplinary boundaries and institutional affiliations.
    
    \item \textbf{Can similarity-based features predict future coauthorship with sufficient accuracy to provide actionable recommendations?} This requires evaluating whether shared publication venues and paper overlap serve as reliable indicators of collaboration potential, and determining appropriate similarity metrics and thresholds.
    
    \item \textbf{How do collaboration patterns differ across research domains and career stages?} Understanding heterogeneity in academic networks helps inform whether a single prediction model suffices or domain-specific customization is necessary.
\end{enumerate}
...
While this work demonstrates the feasibility and utility of knowledge graph approaches for research community analysis, several limitations should be acknowledged:

\begin{itemize}
    \item \textbf{Single-source data:} The dataset comes from a single bibliographic source with potential selection bias. Integrating multiple sources (institutional repositories, publisher databases, citation indices) would provide more complete coverage but introduces data integration challenges.
    
    \item \textbf{Snapshot analysis:} Current implementation treats the collaboration network as static, while real research communities evolve continuously. Temporal dynamics—how collaborations form, dissolve, and shift over time—are not captured.
    
    \item \textbf{Publication-based collaboration only:} The graph captures formal coauthorship but misses informal collaborations, mentorship relationships, grant partnerships, and other forms of research interaction that don't result in joint publications.
    
    \item \textbf{Limited evaluation methodology:} Link prediction accuracy is difficult to validate without ground truth future collaborations. Current evaluation relies on held-out data from the same time period rather than true temporal prediction.
    
    \item \textbf{Domain generalizability:} The dataset appears to emphasize engineering and applied sciences. Collaboration patterns in humanities, social sciences, or theoretical fields may differ substantially.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Chapter 1: Introduction} – Establishes the background, motivation, scope, research questions, and contributions of the project.
    \item \textbf{Chapter 2: Theoretical Foundations} – Provides essential background on graph theory, community detection algorithms, and similarity metrics.
    \item \textbf{Chapter 3: Related Work} – Reviews key academic papers and existing technologies relevant to our domain.
    \item \textbf{Chapter 4: Proposed System Design} – Presents the initial design strategy and planned components.
    \item \textbf{Chapter 5: Tools and Technologies Evaluated} – Details the tools, libraries, and models evaluated during development.
    \item \textbf{Chapter 6: Methodology} – Describes the dataset, Neo4j implementation, and algorithmic approaches.
    \item \textbf{Chapter 7: System Implementation} – Provides detailed implementation specifics including preprocessing, graph construction, and algorithm execution.
    \item \textbf{Chapter 8: Results} – Presents quantitative and qualitative findings from the analysis.
    \item \textbf{Chapter 9: Discussion} – Interprets results, discusses limitations, and compares with related work.
    \item \textbf{Chapter 10: Future Work} – Outlines potential enhancements and research directions.
    \item \textbf{Chapter 11: Conclusion} – Summarizes outcomes and contributions.
\end{itemize}
...
This chapter provides essential background on the theoretical concepts and algorithms that underpin our research community knowledge graph system. Understanding these foundations is crucial for appreciating the design choices and interpreting the results presented in later chapters.

\section{Graph Theory Fundamentals}

A \textbf{graph} $G = (V, E)$ consists of a set of vertices (nodes) $V$ and a set of edges (relationships) $E \subseteq V \times V$. In our collaboration network:

\begin{itemize}
    \item \textbf{Nodes} represent entities: authors, papers, journals, and coauthorship events
    \item \textbf{Edges} represent relationships: WROTE, PUBLISHED\_IN, COAUTHORED
    \item \textbf{Properties} store attributes on both nodes and edges
\end{itemize}

\subsection{Graph Types}

\textbf{Directed vs. Undirected Graphs:} In a directed graph, edges have orientation (A $\rightarrow$ B is different from B $\rightarrow$ A). Our WROTE and PUBLISHED\_IN relationships are directed. In an undirected graph, edges are symmetric. Coauthorship is inherently undirected—if A coauthored with B, then B coauthored with A.

\textbf{Bipartite Graphs:} A bipartite graph has vertices divided into two disjoint sets such that edges only connect vertices from different sets. The author-paper subgraph is bipartite: authors connect to papers, but not directly to other authors (except through papers).

\textbf{Weighted Graphs:} Edges can have weights representing strength or frequency. We could weight coauthorship edges by number of shared papers, though our current implementation treats edges as unweighted.

\subsection{Graph Metrics}

\textbf{Degree:} The degree $d(v)$ of a node $v$ is the number of edges incident to it. In directed graphs, we distinguish in-degree (incoming edges) and out-degree (outgoing edges). For authors, degree represents the number of papers they've written.

\textbf{Path:} A path is a sequence of edges connecting two nodes. Path length is the number of edges traversed. Shortest paths reveal how closely connected two researchers are through their collaboration network.

\textbf{Density:} Graph density measures how many edges exist relative to the maximum possible:
$$\text{Density} = \frac{|E|}{|V|(|V|-1)/2}$$
for undirected graphs. Dense networks indicate highly interconnected communities.

\textbf{Clustering Coefficient:} Measures the degree to which nodes cluster together. For a node $v$, it's the proportion of connections between its neighbors:
$$C(v) = \frac{\text{actual triangles around } v}{\text{possible triangles around } v}$$
High clustering indicates tightly-knit research groups.

\section{Community Detection Theory}

Community detection identifies groups of nodes that are more densely connected internally than to the rest of the network. This reveals natural clusters—in our case, research communities.

\subsection{Modularity}

Modularity quantifies the quality of a network partition into communities. It compares actual edge density within communities to the expected density in a random network:

$$Q = \frac{1}{2m} \sum_{ij} \left[ A_{ij} - \frac{k_i k_j}{2m} \right] \delta(c_i, c_j)$$

where:
\begin{itemize}
    \item $A_{ij}$ is the adjacency matrix (1 if edge exists between nodes $i$ and $j$, 0 otherwise)
    \item $k_i$ and $k_j$ are the degrees of nodes $i$ and $j$
    \item $m$ is the total number of edges
    \item $c_i$ and $c_j$ are the communities of nodes $i$ and $j$
    \item $\delta(c_i, c_j) = 1$ if $c_i = c_j$, otherwise 0
\end{itemize}

Modularity ranges from -1 to 1. Values above 0.3 typically indicate significant community structure. Higher modularity means stronger community separation.

\subsection{Louvain Algorithm}

The Louvain algorithm is a greedy optimization method for detecting communities by maximizing modularity. It operates in two phases:

\textbf{Phase 1 - Local Moving:}
\begin{enumerate}
    \item Start with each node in its own community
    \item For each node $i$, calculate modularity gain from moving $i$ to each neighbor's community
    \item Move $i$ to the community yielding maximum positive gain
    \item Repeat until no moves improve modularity (local optimum)
\end{enumerate}

\textbf{Phase 2 - Aggregation:}
\begin{enumerate}
    \item Build a new network where communities become super-nodes
    \item Edge weights between super-nodes equal sum of weights between their constituent nodes
    \item Apply Phase 1 to this coarsened network
\end{enumerate}

The algorithm alternates between phases until modularity stabilizes. This hierarchical approach is computationally efficient ($O(n \log n)$) and produces high-quality partitions.

\textbf{Advantages:}
\begin{itemize}
    \item Fast execution on large networks
    \item Hierarchical community structure
    \item High modularity scores
    \item No need to specify number of communities in advance
\end{itemize}

\textbf{Limitations:}
\begin{itemize}
    \item Resolution limit—may miss small communities
    \item Non-deterministic (depends on node ordering)
    \item Tends to produce unbalanced community sizes
\end{itemize}

\section{Similarity Metrics for Link Prediction}

Link prediction estimates the likelihood of future edges between node pairs. Similarity metrics quantify how "close" two nodes are based on their features or network position.

\subsection{Cosine Similarity}

Cosine similarity measures the angular similarity between two feature vectors. Given authors represented as binary vectors indicating their papers and journals:

$$\text{Cosine}(A, B) = \frac{A \cdot B}{\|A\| \|B\|} = \frac{\sum_{i} A_i B_i}{\sqrt{\sum_{i} A_i^2} \sqrt{\sum_{i} B_i^2}}$$

Cosine similarity ranges from 0 (orthogonal, no overlap) to 1 (identical). It captures directional alignment—whether two authors point toward the same regions of the publication space.

\textbf{Geometric Interpretation:} If we plot authors in a high-dimensional space where each dimension represents a paper or journal, cosine similarity is the cosine of the angle between their position vectors. Small angles mean similar research profiles.

\textbf{Advantages:}
\begin{itemize}
    \item Normalized, making it invariant to vector magnitude (prolific vs. less prolific authors)
    \item Efficient computation via matrix operations
    \item Intuitive interpretation
\end{itemize}

\textbf{Limitations:}
\begin{itemize}
    \item Ignores feature absence (zeros in both vectors contribute nothing)
    \item May not capture asymmetric relationships
\end{itemize}

\subsection{Jaccard Similarity}

Jaccard similarity is a set-based metric measuring overlap between two sets:

$$\text{Jaccard}(A, B) = \frac{|A \cap B|}{|A \cup B|} = \frac{\text{features in common}}{\text{total distinct features}}$$

For authors, $A$ and $B$ are sets of papers and journals they've published in.

Jaccard ranges from 0 (disjoint sets) to 1 (identical sets). Unlike cosine, it directly measures set overlap without considering vector direction.

\textbf{Example:} Author A published in \{Journal X, Journal Y, Journal Z\}. Author B published in \{Journal Y, Journal Z, Journal W\}. 
$$\text{Jaccard}(A, B) = \frac{|\{Y, Z\}|}{|\{X, Y, Z, W\}|} = \frac{2}{4} = 0.5$$

\textbf{Advantages:}
\begin{itemize}
    \item Simple, interpretable
    \item Treats all features equally
    \item Works well for sparse binary data
\end{itemize}

\textbf{Limitations:}
\begin{itemize}
    \item Sensitive to rare features
    \item Doesn't weight features by importance
    \item Biased toward high-overlap pairs even if overlap is small fraction of total
\end{itemize}

\subsection{Hybrid Approach}

Our system combines both metrics:
$$\text{Score}(A, B) = \frac{\text{Cosine}(A, B) + \text{Jaccard}(A, B)}{2}$$

This balances directional similarity (cosine) with set overlap (Jaccard), providing more robust predictions than either metric alone. Pairs scoring high on both metrics are strong collaboration candidates.

\section{Link Prediction in Networks}

Link prediction estimates future or missing edges. Beyond similarity metrics, graph structure itself provides signals:

\textbf{Common Neighbors:} If A and B share many neighbors, they're likely to connect. In collaboration networks, shared coauthors increase collaboration probability.

\textbf{Preferential Attachment:} High-degree nodes attract more edges (the rich get richer). Established researchers may attract more collaborators.

\textbf{Adamic-Adar Index:} Weights common neighbors by their rarity:
$$AA(A, B) = \sum_{u \in \Gamma(A) \cap \Gamma(B)} \frac{1}{\log |\Gamma(u)|}$$
where $\Gamma(v)$ is the set of neighbors of $v$. Rare shared connections are more predictive than common ones.

Our feature-based approach using publication patterns captures domain-specific signals beyond pure topology, making predictions more relevant to actual research compatibility.
...
\subsection*{Paper 8: Evolving knowledge graph
representation learning with multiple attention strategies for citation recommendation system}
To get better results from EKGE (Evolving Knowledge Graph Embedding) feature extraction, Liu et al. incorporate a multiple-attention mechanism into their framework for citation recommendation. The outcome was evident, with the model able to generate recommendation lists more closely aligned with user requirements. Experimental evaluations on citation recommendation datasets reported that EKGE improved accuracy and achieved approximately a 1.13\% improvement in predictive performance compared to baseline KGE methods. While this improvement may seem modest numerically, in large-scale recommendation systems where users interact with thousands of potential citations, even small accuracy gains translate to substantially better user experience and more relevant suggestions.
...
This section outlines the proposed design of the knowledge graph generation and query system based on the understanding developed during Phase 1. It includes the system’s objective, architecture, core modules, key technologies considered, and potential implementation challenges.

\section{Objective of the System}
    
The objective of the system is to automatically extract semantic entities and relationships from unstructured research papers (in PDF format) and convert them into structured knowledge graphs. These graphs should be both machine-readable and human-explorable. In Phase 2, users will be able to interact with the graph using natural language queries to explore research concepts, identify method connections, and trace concept dependencies across academic literature.
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering \textbf{[Figure 5.1: Proposed System Flow Graph]} \\ \vspace{0.5cm} \textit{(Flow diagram to be inserted by instructor showing: CSV Input $\rightarrow$ Data Preprocessing $\rightarrow$ Neo4j Import $\rightarrow$ Community Detection $\rightarrow$ Link Prediction $\rightarrow$ Visualization \& Querying)} \\ \vspace{0.5cm}}}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering \textbf{[Figure 5.2: Module Architecture Diagram]} \\ \vspace{0.5cm} \textit{(Diagram showing modules: Data Preprocessing Module, Graph Construction Module, Community Detection Module, Link Prediction Module, Query \& Visualization Module with their interconnections and data flows)} \\ \vspace{0.5cm}}}
...
\textbf{Sample Query (Find cross-community collaborations):}
\begin{verbatim}
MATCH (a1:Author)-[:COAUTHORED]->(c:Coauthorship)
      <-[:COAUTHORED]-(a2:Author)
WHERE a1.community <> a2.community
RETURN a1.name, a1.community, a2.name, a2.community, c.title
\end{verbatim}
...
\chapter{Tools and Technologies Evaluated}

\section{PDF Parsing Tools}

To extract raw text from research papers, several PDF parsing libraries were evaluated. Among them, pdfminer.six and PyMuPDF (fitz) were found to be the most effective. pdfminer.six offers detailed access to text layout and structure, while PyMuPDF is faster and easier to integrate in lightweight workflows. Both tools demonstrated sufficient accuracy for handling research papers with standard academic formatting, although some papers with heavy formulas or images may require additional preprocessing or OCR support.

\section{Language Models for Information Extraction}

Various LLMs and transformer-based models were studied for their ability to extract structured information from unstructured text. OpenAI's GPT-3.5 and GPT-4, Google Gemini Pro, and Hugging Face's RoBERTa-based QA models were all tested with structured prompts. GPT-based models performed exceptionally well in understanding context and generating structured fields like title, authors, and methodology. Hugging Face's roberta-base-squad2 offered an open-source, offline-friendly alternative suitable for environments with resource or API limitations.

\section{Graph Construction and Visualization}

To build and visualize knowledge graphs, tools like NetworkX and Neo4j were explored. NetworkX is lightweight and Python-native, suitable for small to medium-sized graphs. Neo4j, a graph database with Cypher query support, enables efficient storage, advanced querying, and browser-based visualization. It is especially useful when graphs grow in complexity and size, making it a potential candidate for Phase 2 implementation.


\section{Querying Techniques and Interfaces}

Natural language querying and semantic search over knowledge graphs are important for accessibility. Early explorations were done into using LangChain and LlamaIndex for connecting LLMs with structured data backends. Cypher (Neo4j’s query language) and SPARQL were reviewed as backend options for precise querying. The plan is to implement a hybrid interface where natural language queries are interpreted and translated into graph queries in real time.

\section{Tool Comparison Summary}

The table below summarizes the key tools explored during Phase 1 and evaluates their suitability for various stages of the proposed system.

\begin{table}[htpb]
\centering
\caption{Summary of Evaluated Tools and Technologies}
\begin{tabular}{|p{4cm}|p{4cm}|p{6.7cm}|}
\hline
\textbf{Component} & \textbf{Tool / Model} & \textbf{Notes} \\
\hline
PDF Parsing & pdfminer.six, PyMuPDF & Accurate and scriptable tools, suitable for extracting text from research PDFs with structured formatting. \\
\hline
Information Extraction & GPT-3.5, Gemini, RoBERTa QA (Hugging Face) & GPT-based models offer excellent contextual understanding; RoBERTa QA is a strong open-source alternative for offline use. \\
\hline
Graph Construction & NetworkX, Neo4j & NetworkX is ideal for prototyping and smaller graphs; Neo4j provides scalable graph storage and interactive querying. \\
\hline
Querying Layer & Cypher, LangChain, SPARQL & Cypher enables complex graph traversal; LangChain enables LLM-based query transformation; SPARQL supports RDF-style queries. \\
\hline
\end{tabular}
...
\begin{table}[htpb]
\centering
\caption{Comparison of Language Models for Information Extraction}
\begin{tabular}{|p{3cm}|p{2cm}|p{2.2cm}|p{3cm}|p{3.5cm}|}
\hline
\textbf{Model / Tool} & \textbf{Type} & \textbf{Access Mode} & \textbf{Strengths} & \textbf{Limitations} \\
\hline
GPT-3.5 / GPT-4 & LLM (API) & Online & Strong semantic understanding and fluent generation & Rate limits, cost, external API dependency \\
\hline
Gemini Pro & LLM (API) & Online & Competitive performance and integration with Google services & API quota limits, still under evaluation \\
\hline
Hugging Face QA (RoBERTa) & Transformer (open-source) & Offline & Lightweight, customizable, privacy-friendly & Needs tuning for academic domains, limited context window \\
\hline
\end{tabular}
...
This chapter provides detailed technical documentation of how the research collaboration knowledge graph was implemented, from raw CSV data to a queryable Neo4j database with community detection and link prediction capabilities. We describe the data preprocessing pipeline, graph construction process, algorithm implementations, and performance optimizations.
...
\textit{Note: Dataset description has been moved to Chapter 4 (Proposed System Design) to logically group data specifications with system architecture. This chapter focuses on the algorithmic and implementation methodology.}

\subsection{How the Dataset Was Used in This Project}

\subsubsection{Data Import}

The import script (\texttt{import\_to\_neo4j.py}) creates:

\begin{itemize}
    \item \textbf{Author nodes:} one per unique author name string.
    \item \textbf{Paper nodes:} one per title, with a \texttt{document\_type} property when available.
    \item \textbf{Journal nodes:} one per source title.
    \item \textbf{Relationships:}
    \begin{itemize}
        \item Author -[:WROTE]$\rightarrow$Paper
        \item Paper -[:PUBLISHED\_IN]$\rightarrow$Journal
        \item Author -[:COAUTHORED]$\rightarrow$Coauthorship node for each paper
    \end{itemize}
\end{itemize}

\subsubsection{Coauthorship Prediction}

The \texttt{predict\_coauthorship.py} script extracts Author--Paper--Journal triples from Neo4j and builds binary feature vectors (one-hot over papers and journals) to compute cosine and Jaccard similarity between authors, producing a ranked list of likely future coauthorships.
    
\section{Neo4j}

Neo4j is a native graph database designed to store and query graph-shaped data (nodes, relationships, and properties) efficiently. It uses the labeled property graph model: nodes have labels and properties, relationships connect nodes and can also have properties. Neo4j exposes a declarative query language, Cypher, for graph pattern matching and manipulation. It also provides the Graph Data Science (GDS) library for graph algorithms (community detection, centrality, link prediction) and integrations for visualization (Neo4j Browser, Bloom) and drivers for Python, Java, etc.

\subsection{How We Used Neo4j in This Project (Detailed)}

\subsubsection{Data Model Chosen}

\paragraph{Node Types (Labels):}

\begin{itemize}
    \item \textbf{Author:} One node per unique author name (property: \texttt{name}; additional properties may include \texttt{communityId} after running GDS).
    
    \item \textbf{Paper:} One node per unique title (property: \texttt{title}, optional: \texttt{document\_type}).
    
    \item \textbf{Journal:} One node per source title (property: \texttt{name}).
    
    \item \textbf{Coauthorship:} One node per paper capturing the coauthorship event (properties: \texttt{title}, \texttt{journal}, \texttt{document\_type}). We used a Coauthorship node to make coauthorships explicit and to avoid overly dense fully-connected author subgraphs.
\end{itemize}

\paragraph{Relationships:}

\begin{itemize}
    \item \texttt{(Author)-[:WROTE]->(Paper)}: author wrote/credited on the paper.
    
    \item \texttt{(Paper)-[:PUBLISHED\_IN]->(Journal)}: paper was published in the journal.
    
    \item \texttt{(Author)-[:COAUTHORED]->(Coauthorship)}: author is one of the coauthors on that coauthorship event (paper).
    
    \item Optionally earlier we experimented with \texttt{(Author)-[:COAUTHOR]->(Author)}, but later removed these direct author-to-author links to keep the model clearer and avoid duplication.
\end{itemize}

\paragraph{Properties Used:}

\begin{itemize}
    \item \texttt{Paper.document\_type}: stored if available in the CSV.
    
    \item \texttt{Coauthorship.title}, \texttt{Coauthorship.journal}, \texttt{Coauthorship.document\_type}: stored to preserve provenance and make the coauthorship node self-contained.
    
    \item \texttt{communityId}: written by GDS after running community detection.
\end{itemize}

\subsubsection{Import Process}

The Python script \texttt{import\_to\_neo4j.py} reads \texttt{research\_csv.csv}, splits the Authors string by commas, merges Journal and Paper nodes, creates Author nodes (MERGE to avoid duplicates), creates Paper->Journal relationships, creates Coauthorship nodes (one per paper), and connects authors to coauthorship nodes via \texttt{COAUTHORED} relationships. Import uses the official \texttt{neo4j} Python driver and \texttt{MERGE} to avoid creating duplicate nodes/rels in idempotent runs.

\subsubsection{Graph Algorithms \& Analysis}

\paragraph{Community Detection:}

We used Neo4j GDS (Louvain or Label Propagation) to detect communities of authors. For this we projected an author network (authors connected via coauthored papers). There were two approaches:

\begin{itemize}
    \item Project direct author-to-author graph (COAUTHOR edges) into GDS and run Louvain.
    
    \item Alternatively, project a two-mode graph (Author and Coauthorship) and run projection rules to derive an Author-only graph for GDS.
\end{itemize}

\paragraph{Link Prediction / Similarity:}

We exported Author-Paper relationships and computed cosine and Jaccard similarity in Python (\texttt{predict\_coauthorship.py}) to rank likely future collaborations. This is a simple feature-based link-prediction approach; Neo4j GDS also provides link prediction algorithms (e.g., common neighbors, Adamic-Adar, node-embeddings) if preferred.


\section{Results}

\subsection{Graph Statistics and Network Topology}
...
\subsubsection{Journal Publication Distribution}
...
\subsubsection{Coauthorship Patterns}
...
\subsection{Author, Paper, Journal Network (LIMIT 20)}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{media/Picture1.png}
...
\subsection{Author, Paper, Journal Network (LIMIT 100)}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{media/Picture2.png}
...
\subsection{Community Detection (LIMIT 100)}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{media/Picture3.png}
...
\subsection{Community Detection (LIMIT 300)}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{media/Picture4.png}
...
\subsection{Community Detection Analysis}
...
\subsubsection{Community Structure Statistics}
...
\subsubsection{Characterization of Major Communities}
...
\subsubsection{Cross-Community Collaboration}
...
\subsubsection{Similarity Score Distributions}
...
\subsubsection{Top Predicted Collaborations}
...
\subsection{Paper and Publication Venue Network}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{media/Picture5.png}
...
% \begin{enumerate}
%     \item Q. Wang, Z. Mao, B. Wang, and L. Guo, “Knowledge Graph Embedding: A Survey of Approaches and Applications,” \textit{IEEE Transactions on Knowledge and Data Engineering}, vol. 29, no. 12, pp. 2724–2743, Dec. 2017. doi:10.1109 \\ /TKDE.2017.2754499.

%     \item J. Pujara, H. Mia, L. Getoor, and W. Cohen, “Knowledge Graph Identification,” in \textit{Proc. 13th Int. Semantic Web Conf. (ISWC)}, Sydney, Australia, 2013. doi:10.1007/978-3-642-41338-4\_20.

%     \item Z. Liu, J. Zhou, B. Wang, et al., “Construction and Application of a Knowledge Graph for Surveying and Mapping,” \textit{ISPRS International Journal of Geo-Information}, vol. 8, no. 3, p. 112, 2019. doi:10.3390/ijgi8030112.

%     \item L. Pan, X. Wang, and Y. Zhang, “KnowEdu: A System to Construct Knowledge Graph for Education,” in \textit{Proc. 27th ACM Int. Conf. on Information and Knowledge Management (CIKM)}, Turin, Italy, 2018. doi:10.1145/3269206.3269255.

%     \item Gera, S., & Sinha, A. (2018, April). Influence Dynamics in Social Networks. In Proceedings of 3rd International Conference on Internet of Things and Connected Technologies (ICIoTCT) (pp. 26-27).

%     \item Ali, Z., Qi, G., Kefalas, P., Abro, W. A., & Ali, B. (2020). A graph-based taxonomy of citation recommendation models. Artificial Intelligence Review, 53(7), 5217-5260.

%     \item Verma, S., Bhatia, R., Harit, S., & Batish, S. (2023). Scholarly knowledge graphs through structuring scholarly communication: a review. Complex & intelligent systems, 9(1), 1059-1095.

%     \item Liu, J. C., Chen, C. T., Lee, C., & Huang, S. H. (2024). Evolving knowledge graph representation learning with multiple attention strategies for citation recommendation system. ACM Transactions on Intelligent Systems and Technology, 15(2), 1-26.

%     \item Kejriwal, M. (2022). Knowledge graphs: A practical review of the research landscape. Information, 13(4), 161.Dessí, D., Osborne, F., Recupero, D. R., Buscaldi, D., & Motta, E. (2022). scientific knowledge graphs in the computer science domain”. Future Generation Computer Systems, 116:136– 149, 2020.

%     \item Wang, X., Xu, H., Tan, W., Wang, Z., & Xu, X. (2020, August). Scholarly paper recommendation via related path analysis in knowledge graph. In 2020 International Conference on Service Science (ICSS) (pp. 36-43). IEEE.
% \end{enumerate}
...
Knowledge graphs provide a natural and powerful representation for academic collaboration networks, enabling rich queries, sophisticated algorithms, and intuitive visualizations. This project demonstrates that practical systems can be built using open-source tools (Python, Neo4j, scikit-learn) and standard algorithms (Louvain, cosine similarity), achieving meaningful results on real-world data.

The strong community structure we observed (modularity 0.73) confirms that research is organized into distinct collaborative clusters, while the presence of boundary-spanning researchers suggests opportunities for strategic network interventions to foster cross-disciplinary collaboration.

Future enhancements—particularly temporal analysis, citation network integration, and content-aware prediction—promise to make such systems even more valuable tools for navigating the complex landscape of modern research collaboration. As academic output continues to grow exponentially, intelligent systems for organizing and discovering knowledge will become increasingly essential.

This work establishes a foundation for building next-generation scholarly knowledge graphs that not only organize information but actively facilitate the collaborative processes through which knowledge is created.
...
\chapter{Key Python Code Snippets}

\section{Author Name Normalization}
...
def clean_author(author):
    """
    Normalize author names for consistent entity resolution.
    
    Args:
        author (str): Raw author name string
        
    Returns:
        str: Cleaned and normalized author name
    """
    # Remove leading/trailing whitespace
    author = author.strip()
...
\section{Binary Feature Matrix Construction}

\begin{verbatim}
from sklearn.preprocessing import MultiLabelBinarizer
import numpy as np

def build_feature_matrix(author_features):
    """
    Convert author publication profiles to binary feature matrix.
    
    Args:
        author_features (dict): Maps author names to sets of 
                                papers and journals
                                
    Returns:
        tuple: (feature_matrix, author_list, mlb)
    """
    authors = list(author_features.keys())
    
    # Build feature lists combining papers and journals
    feature_lists = [
        list(author_features[a]['papers']) + 
        list(author_features[a]['journals'])
        for a in authors
    ]
    ...
    return feature_matrix, authors, mlb
\end{verbatim}
...
def compute_hybrid_similarity(feature_matrix):
    """
    Compute hybrid similarity combining cosine and Jaccard.
    
    Args:
        feature_matrix (np.ndarray): Binary feature matrix
        
    Returns:
        np.ndarray: Hybrid similarity matrix
    """
    # Cosine similarity
    cosine_sim = cosine_similarity(feature_matrix)
    
    # Jaccard similarity
    intersection = feature_matrix.dot(feature_matrix.T)
    row_sums = np.array(feature_matrix.sum(axis=1))
    union = row_sums + row_sums.T - intersection
    jaccard_sim = intersection / union
    
    # Hybrid average
    hybrid_sim = (cosine_sim + jaccard_sim) / 2
    
    return hybrid_sim
\end{verbatim}

\chapter{Complete Dataset Statistics}

\section{Degree Distribution Details}

\begin{table}[h!]
\centering
\caption{Complete Author Degree Distribution}
\begin{tabular}{|r|r|r|}
\hline
\textbf{Papers} & \textbf{Authors} & \textbf{Percentage} \\
\hline
1 & 876 & 28.7\% \\
2 & 562 & 18.4\% \\
3 & 387 & 12.7\% \\
...
31+ & 18 & 0.6\% \\
\hline
\textbf{Total} & \textbf{3,057} & \textbf{100.0\%} \\
\hline
\end{tabular}
\label{tab:full_degree_dist}
\end{table}

\section{Journal Distribution Details}

\begin{table}[h!]
\centering
\caption{Top 20 Publication Venues}
\small
\begin{tabular}{|l|r|}
\hline
\textbf{Venue} & \textbf{Papers} \\
...
Journal of Physics: Conference Series & 32 \\
Lecture Notes in Electrical Engineering & 32 \\
Springer Proceedings in Mathematics and Statistics & 28 \\
Advances in Intelligent Systems and Computing & 25 \\
International Journal of Engineering and Technology & 22 \\
...
Wireless Personal Communications & 14 \\
Multimedia Tools and Applications & 13 \\
International Journal of Intelligent Systems & 12 \\
Expert Systems with Applications & 12 \\
Journal of Intelligent \& Fuzzy Systems & 11 \\
Soft Computing & 11 \\
Applied Soft Computing & 10 \\
Cluster Computing & 10 \\
\hline
\end{tabular}
\label{tab:top20_venues}
\end{table}
...
\bibliographystyle{unsrt}  % or 'plain', 'apalike', 'unsrt', etc.
